...
export default (({ inFiles, outFiles }) => {
  ...
  const importmap = $(<script type="importmap">{JSON.stringify({
    "imports": Object.fromEntries(Object.keys(pkg.dependencies).map(name =>
      [name, `https://cdn.jsdelivr.net/npm/${name}@latest/+esm`]
    ))
  })}</script>)

  const iconlink = <link rel="shortcut icon" href={`data:image/svg+xml,${encodeURIComponent(icon)}`} />
  const headers = [...datas, ...modules, iconlink, importmap]
